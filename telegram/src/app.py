...
def prepare_telegram_content(text: str) -> str:
        ...
        def replace(match: re.Match[str]) -> str:
            ...
            if raw.startswith("</"):
                if tag in allowed and stack and stack[-1] == tag:
                    stack.pop(); return f"</{tag}>"
                return ""
            if tag not in allowed: return ""
            if tag == "a":
                href = re.search(r'href=["\']([^"\']+)["\']', attrs, re.I)
                if not href or not href.group(1).startswith(("http://", "https://")): return ""
                stack.append(tag); return f'<a href="{escape(href.group(1), quote=True)}">'
            stack.append(tag); return f"<{tag}>"
    ...
    for line in text.splitlines():
        if line.strip().startswith("```"):
            if in_code:
                lines.append(f"<pre><code>{escape(chr(10).join(code_lines))}</code></pre>"); code_lines = []
    ...
    if in_code: lines.append(f"<pre><code>{escape(chr(10).join(code_lines))}</code></pre>")
    return "\n".join(lines)
...
def webapp_url() -> str | None:
    ...
    parsed = urlparse(raw)
    if parsed.scheme != "https" or not parsed.netloc or "." not in (parsed.hostname or ""): return None
    return raw
...
async def main() -> None:
    ...
    async def start(message: types.Message):
        if not is_admin(message.from_user.id):
            await message.answer("Access is allowed only for administrators."); return
        url = webapp_url()
        if not url:
            await message.answer("WebApp is currently unavailable."); return
    ...
    async def messages(message: types.Message):
        ...
        if text.startswith("/ask"):
            prompt = text[4:].strip()
            if not prompt:
                await message.answer("Please provide a request after the /ask command"); return
    ...
    async def callbacks(callback: types.CallbackQuery):
        ...
        if data.startswith("approve:"):
            draft_id = int(data.split(":", 1)[1]); draft = await fetch_draft(draft_id)
            if not draft or draft.get("status") != "pending":
                await callback.answer("Draft not found or already processed.", show_alert=True); return
            ...
            if not draft or draft.get("status") != "pending":
                await callback.answer("Draft not found or already processed.", show_alert=True); return
            try:
                await update_draft_status(draft_id, "rejected"); await callback.answer("Draft rejected.")
            finally:
                try: await bot.delete_message(callback.message.chat.id, callback.message.message_id)
                except Exception: pass
        elif data.startswith("delete:"):
            draft_id = int(data.split(":", 1)[1])
            try: await delete_draft(draft_id); await callback.answer("Draft deleted.")
            except Exception: await callback.answer("Failed to delete draft.", show_alert=True)
            finally:
                try: await bot.delete_message(callback.message.chat.id, callback.message.message_id)
                except Exception: pass
        elif data.startswith("regenerate:"):
            schedule_id = int(data.split(":", 1)[1]); schedule = await fetch_schedule(schedule_id)
            if not schedule or not schedule.get("is_active", False):
                await callback.answer("Schedule is unavailable.", show_alert=True); return
    ...
    try:
        ...
        schedule_task.cancel(); publication_task.cancel()
