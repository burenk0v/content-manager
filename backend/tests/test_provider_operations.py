...
def test_provider_operation_key_is_reused_across_retry_attempts():
    ...
    first = client.post(f"/content/publications/{publication['id']}/claim", json={"worker_id": "worker-a"}, headers=HEADERS)
    ...
    failed = client.post(f"/content/publications/{publication['id']}/fail", json={"worker_id": "worker-a", "processing_token": token, "error_message": "temporary provider failure", "retry": True}, headers=HEADERS)
...
def test_ambiguous_provider_outcome_is_terminal_until_reconciliation():
    ...
    failed = client.post(f"/content/publications/{publication['id']}/fail", json={"worker_id": "worker-a", "processing_token": token, "error_message": "provider outcome unknown", "retry": False}, headers=HEADERS)
    assert failed.status_code == 200
